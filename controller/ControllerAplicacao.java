...
public class ControllerAplicacao implements Initializable {
    ...
    @FXML 
    private ChoiceBox<String> escolhaControleErro; // choice box para escolher os erros

    private String [] nomesControleErro = { "Bit de parirade par" , "Bit de paridade impar" , "CRC" , "Hamming"};

    @FXML 
    private ChoiceBox<String> escolhaEnquadramento; // choice box para escolher os erros

     private String [] nomesEnquadramento = { "Contagem de caracter" , "Inserção de Bytes" , "Inserção de Bits" , "Violação da camada física"};
    ...
    private static int  valorDoErro;
    ...
    public void initialize(URL location, ResourceBundle resources) {
      ...
      sliderErro.setSnapToPixel(true); // faz com que o slider avance apenas de 10 em 10

    valorDoErro = (int) sliderErro.getValue();
    valorErro.setText(Integer.toString(valorDoErro));
       

        botaoEnviar.setOpacity(0); // Define a opacidade inicial do botao
        entradaMensagem.textProperty().addListener((observable, oldValue, newValue) -> {
            if (newValue.isEmpty()) {
                botaoEnviar.setOpacity(0); // Oculta o botao quando o campo esta vazio
            } else {
                botaoEnviar.setOpacity(1); // Exibe o botao quando ha texto
            }
        });
    ...
    escolhaControleErro.getItems().addAll(nomesControleErro); // adiciona os nomes das escolhas no choice box
    // Define o valor inicial do ChoiceBox para evitar -1
    if (!escolhaControleErro.getItems().isEmpty()) {
    escolhaControleErro.getSelectionModel().selectFirst(); // seleciona o primeiro item da lista
      } 

    escolhaEnquadramento.getItems().addAll(nomesEnquadramento);
       if (!escolhaEnquadramento.getItems().isEmpty()) {
   escolhaEnquadramento.getSelectionModel().selectFirst(); // seleciona o primeiro item da lista
      } 
    }
  ...
  *************************************************************** */
    ...
    public void enviarMensagem(MouseEvent event) {
      data.setErro(escolhaControleErro.getSelectionModel().getSelectedIndex()); // passa o index para o datasingleton 
      data.setEnquadramento(escolhaEnquadramento.getSelectionModel().getSelectedIndex());
      System.out.println("Enquadramento escolhido: " + escolhaEnquadramento.getSelectionModel().getSelectedIndex());
      ...
      ondasImagens.clear();
         ...
         startWaveAnimation(); // Inicia a animacao das ondas
    }
  ...
  *************************************************************** */
    private void criarOnda(int i) {
        ...
        adicionarOnda(caminhoImagem, 210, posicaoY, 86, larguraImagem); // Chama o metodo para adicionar a onda
    }
  ...
  *************************************************************** */
    private void startWaveAnimation() {
        stopWaveAnimation(); // Parar qualquer animacao existente antes de iniciar uma nova
  ...
  *************************************************************** */
    ...
    public void voltarTela(MouseEvent event) throws IOException {
        stopWaveAnimation(); // Parar a animacao ao voltar
  ...
  *************************************************************** */
    ...
    /* ***************************************************************
  * Metodo: getValorDoErro
  * Funcao: retornar o valor do erro
  * Parametros: void
  * Retorno: int
  *************************************************************** */
     public static int getValorDoErro() {
      return valorDoErro;
    }
