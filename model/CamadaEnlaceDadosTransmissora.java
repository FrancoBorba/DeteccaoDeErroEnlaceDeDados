...
public class CamadaEnlaceDadosTransmissora {
 ...
 private static DataSingleton data = DataSingleton.getInstance(); // transmite informacao entre as telas
  ...
  public static void camadaEnlaceDadosTransmissora(int quadro[]){ 
    int quadroEnquadrado [];
     quadroEnquadrado = camadaEnlaceDadosEnquadramento(quadro);
     quadroEnquadrado = camadaEnlaceDadosControleDeErro(quadroEnquadrado);

     CamadaFisicaTransmissora.camadaFisicaTransmissora(quadroEnquadrado);

  }

  public static int [] camadaEnlaceDadosEnquadramento(int quadro[]){
   ...
   public static int [] camadaEnlaceDadosControleDeErro(int quadro[]){

    System.out.println("Algortimo de erro transmissão");
 ...
 public static int[] camadaEnlaceDadosEnquadramentoInsercaoDeBytes(int quadro[]) {
  ...
  public static int[] camadaEnlaceDadosTransmissoraControleDeErroBitParidadePar(int quadro[]){
    // na contagem por caracter a cada 4 caracteres digitado ele da erro , provavelmente o erro esta no 
    // enquadramento por caracter , caso eu nao conserte ele da certo para todos os casos que nao for multiplo
    // de 4
    // nao sei por que mas se escolher primeiro outro metodo de enquadramento , depois o contagem por caracter
    // ele volta a funcionar para 4 caracteres e seus multiplos
    int novoQuadro[] = new int[quadro.length];
    int bit = 0;
    int mascara = 1 << 31;
    int contadorDeUm = 0; 
    int cont = 0; // processa os 32 bits dentro do quadri
    int inteiro = 0; // armazena o valor do quadro
    int indiceNvQ = 0;

    for(int i = 0 ; i < quadro.length ; i++){
     
      cont = 0;
      inteiro = quadro[i];  // O inteiro esta recebendo 0 quando no proximo quadro eu insiro apenas 1 letra
      ...
      while (cont < 32) {
       ...
       inteiro <<=1;
        ...
        for(int j = 0 ; j < 7 ; j++){
          bit = (inteiro & mascara) == 0 ? 0 : 1; // Verifica se o MSB e 0 ou 1
          novoQuadro[indiceNvQ] = novoQuadro[indiceNvQ]<<=1;;
          novoQuadro[indiceNvQ] = novoQuadro[indiceNvQ] | bit;
          inteiro <<=1;
        }
        
        // adiciona o bit de paridade
        if(contadorDeUm % 2 == 0){ // quantidade de bits ja eh par
          novoQuadro[indiceNvQ] <<=1;
        } else{ // quantidade de bits impar adiciona um bit 1
         novoQuadro[indiceNvQ] = novoQuadro[indiceNvQ]<<=1;;
         novoQuadro[indiceNvQ] = novoQuadro[indiceNvQ] | 1;
        }
  ...
  public static int[] camadaEnlaceDadosTransmissoraControleDeErroBitParidadeImpar(int quadro[]){
    int novoQuadro[] = new int[quadro.length];
    int bit = 0;
    int mascara = 1 << 31;
    int contadorDeUm = 0; 
    ...
    for(int i = 0 ; i < quadro.length ; i++){
      ...
      while (cont < 32) {
       ...
       inteiro <<=1;
        ...
        // adiciona o bit de paridade
        if(contadorDeUm % 2 == 0){ // quantidade de bits eh par adiciona 1
          novoQuadro[indiceNvQ] <<=1;
          novoQuadro[indiceNvQ] = novoQuadro[indiceNvQ] | 1;
        } else{ // quantidade de bits impar adiciona um bit 1
         novoQuadro[indiceNvQ] = novoQuadro[indiceNvQ]<<=1;;
  ...
  public static int[] camadaEnlaceDadosTransmissoraControleDeErroCRC(int quadro[]){
    int novoQuadro [] = new int[quadro.length*2]; // o dobro do quadro original 
    // pois vou calcular o CRC por quadro logo no primeiro quadro vai os bits originais e no segundo o crc
    int crc = 0;
    int inteiro = 0;
    int indiceNvQ = 0;
    ...
    for(int i = 0 ; i < quadro.length ; i++){
      ...
      crc =  calcularCRC32(quadro[i]); // armazena o crc do quadro

      novoQuadro[indiceNvQ] = inteiro; // armazena o quadro original no index
      novoQuadro[indiceNvQ + 1] = crc; // armazena o crc do quadro anterior no index posterior
...
public static int[] camadaEnlaceDadosTransmissoraControleDeErroCodigoDeHamming(int[] quadro) {
    int nBits = quadro.length;
    int nParidade = 0;

    // Calcular o nuumero de bits de paridade necessarios
    while (Math.pow(2, nParidade) < nBits + nParidade + 1) {
        nParidade++;
    }

    int tamanhoTotal = nBits + nParidade;
    int[] quadroCodificado = new int[tamanhoTotal];

    // Colocar os bits de dados e deixar espaços para os bits de paridade
    int j = 0;
    for (int i = 1; i <= tamanhoTotal; i++) {
        if ((i & (i - 1)) == 0) { // Verifica se a posicao eh potencia de 2
            quadroCodificado[i - 1] = 0; // Reservar para bit de paridade
        } else {
            quadroCodificado[i - 1] = quadro[j++];
        }
    }
    ...
    for (int i = 0; i < nParidade; i++) {
        ...
        for (int m = 1; m <= tamanhoTotal; m++) {
            if (((m >> i) & 1) == 1 && m != posicaoParidade) {
                paridade ^= quadroCodificado[m - 1];
            }
        }
        quadroCodificado[posicaoParidade - 1] = paridade;
    }
